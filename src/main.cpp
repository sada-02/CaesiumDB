...
vector<vector<string>> parseMultipleRESP(const char *str, int len)
{
  ...
  while (i < len)
  {
    ...
    if (str[i] == '*')
    {
      ...
      {
        arraySize = arraySize * 10 + (str[i] - '0');
        i++;
      }
      ...
      for (int j = 0; j < arraySize && i < len; j++)
      {
        if (str[i] == '$')
        {
          i++;
...
{
  if (LISTS.find(str) == LISTS.end() || LISTS[str].size == 0)
...
pair<map<long long, map<long, map<string, string>>>, int> checkIDExists(const string &key, string &id)
{
  ...
  for (const auto &ms : STREAM[key].DATA)
  {
    ...
    else
    {
      for (const auto &seq : ms.second)
      {
        if (seq.first <= seqNum[1] && ms.first == seqNum[0])
          continue;
